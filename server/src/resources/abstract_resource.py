...
class AbstractResource(Resource):
    ...
    def delete(self, **kwargs):
        """
        Base method for implementing HTTP DELETE requests
        Child classes should not re implement this, but the Template method: _do_delete
        :param kwargs: extra kwargs
        :return: the response to send to the client
        """
        return self.__execute_handling(self._do_delete, **kwargs)
    ...
    def _do_delete(self, **kwargs):
        """
        Template method to implement for handling an HTTP DELETE on the resource
        :param kwargs: extra kwargs
        :return: processed response
        """
        raise NotFound()