...
class UserListResource(AbstractUserResource):
    ...
    def _do_get(self):
        users = self._user_serializer.serialize(self._user_manager.get_all_users())
        return users
...
class UserResource(AbstractUserResource):
    ...
    def _do_put(self, user_id):
        user = self._user_serializer.deserialize(self._get_post_data_as_json())
        return self._user_serializer.serialize(self._user_manager.update_user(user_id, user))
