...
class TestDefaultUsersManager:
    ...
    def setUp(self):
        self.__test_user = FakeUser(id=self.__USER_ID,
                                    name="userName",
                                    position=(0,0))
    ...
    def test_add_user_with_same_id(self):
        self.__user_manager.add_user(user_id=self.__USER_ID, user=self.__test_user)
        sameIdUser = FakeUser(id=self.__USER_ID,
                              name="otherUser",
                              position=(1,1))
    ...
    def test_add_multiple_users_and_get_all(self):
        all_users = [FakeUser(id=str(userId), name="userName", position=(0, 0)) for userId in range(100)]
    ...
    def test_update_user(self):
        self.__user_manager.add_user(user_id=self.__USER_ID, user=self.__test_user)
        newUser = FakeUser(id=self.__USER_ID,
                           name="newUserName",
                           position=(1,1))
