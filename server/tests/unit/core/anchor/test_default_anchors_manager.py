...
from tests.unit.test_implementations.implementations import FakeAnchor
...
class TestDefaultAnchorsManager:
    ...
    def setUp(self):
        self.__test_anchor = FakeAnchor(id=self.__ANCHOR_ID,
                                        name="anchorName",
                                        position=(0,0))
    ...
    def test_add_anchor_with_same_id(self):
        self.__anchor_manager.add_anchor(anchor_id=self.__ANCHOR_ID, anchor=self.__test_anchor)
        sameIdAnchor = FakeAnchor(id=self.__ANCHOR_ID,
                                  name="otherAnchor",
                                  position=(1,1))
    ...
    def test_add_multiple_anchors_and_get_all(self):
        all_anchors = [FakeAnchor(id=str(anchorId), name="anchorName", position=(0, 0)) for anchorId in range(100)]
    ...
    def test_update_anchor(self):
        self.__anchor_manager.add_anchor(anchor_id=self.__ANCHOR_ID, anchor=self.__test_anchor)
        newAnchor = FakeAnchor(id=self.__ANCHOR_ID,
                               name="newAnchorName",
                               position=(1,1))
